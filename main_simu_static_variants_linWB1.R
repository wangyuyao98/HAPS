...
if (!file.exists("src/gen_ICML_simu.R")) {
    stop("Please run main_simu_static_variants_linWB1.R from the repository root directory.")
}
...
valid_targets <- c("cox_cox_multiclass_loc0", "rsf_rsf_reg_loc0")
target_args <- intersect(args, valid_targets)
method_args <- intersect(args, method_names)
numeric_args <- setdiff(args, c(valid_targets, method_names))
...
format_static_ipcw_file <- function(tag) {
    file.path(
        out_dir,
        paste0(
            "static_IPCW0_", tag,
...
targets <- list(
    cox_cox_multiclass_loc0 = list(
        ...
        base_dynamic_file = format_dynamic_file("dCP_IPCW", "cox", "cox", "cox", "xgb_multiclass", 0),
        static_source = "static_ready",
        static_file = format_static_ipcw_file("cox_cox_Scox_Xixgb_multiclass_loc0_Gtauone"),
        model.pred = "cox",
        model.C = "cox"
    ),
    rsf_rsf_reg_loc0 = list(
        tag = "rsf_rsf_Srsf_Xixgb_reg_loc0_Gtauone",
        base_dynamic_file = format_dynamic_file("dCP_IPCW_new", "rsf", "rsf", "rsf", "xgb_reg", 0),
        static_source = "static_ready",
        static_file = format_static_ipcw_file("rsf_rsf_Srsf_Xixgb_reg_loc0_Gtauone"),
        model.pred = "rsf",
        model.C = "rsf"
    )
)
...
derive_ipcw_trunc <- function(target_name, target) {
    ...
    cfg$source_script <- "main_simu_static_variants_linWB1.R"
